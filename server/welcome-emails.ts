...
import { getWelcomeLetter } from './welcomeLetters';
...
• Payment: 12 monthly payments of $1,000
...
• You'll schedule your first session via [scheduling link TBD]
• All sessions are recorded and available to you for the duration of your mentorship
• Payment: 6 monthly payments of $1,500
...
export function getWelcomeEmail(productId: string): WelcomeEmailTemplate | null {
  // Check if we have a welcome letter template first (new system)
  const welcomeLetter = getWelcomeLetter(productId);
  if (welcomeLetter) {
    return welcomeLetter;
  }
  
  // Fall back to legacy welcome emails
